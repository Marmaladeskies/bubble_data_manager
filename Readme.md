...
- Glitch-free, responsive UX for viewing and editing data tables that is much better than what you can build on Bubble. Fully view and edit your database on mobile. No janky nonsense UI. Uses at least 10x less RAM than a bubble editor window in your browser and has more features:
  + Columns can be reordered and their position can be saved permanently
  + Quickly bulk edit a column in multiple rows (or every row of a constraint view)
  + Easily delete or duplicate multiple rows of data
  + Use today's date to create dynamic constraints that can be saved to always use the current date
  ...
  + Automatically detects your app's data types and fields and populates the table and sidebar for you. Very little setup required.
  + ...And most importantly: you have all of this code, with no weird front-end limitations, so adding or making changes to this (with your favorite AI) is easier, faster, and more customizable than building in Bubble
